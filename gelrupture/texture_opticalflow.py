...
from spatial_domain_toolbox import memory_efficient
...
def normalize_stack(stack, px, preblur=1., profile=None):
    if profile is None:
        profile = np.ma.array(stack, mask=stack>254).mean((1,2)).astype(np.float32)
    profile = np.where(1+profile**2>1, profile, 1)
    f0 = gaussian_filter(stack.astype(np.float32), preblur*px[0]/px[::-1], output=np.float32)
    f0 /= profile[:,None,None]
    return (f0/f0.max()*255).astype(np.float32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--poly_radius', type=int, default=4, help='The radius of the neighbourhood over which the image is approxiated by a second order polynomial.')
    parser.add_argument('--radius',type=int, default=8, help='The radius in X and Y of the window over wich the optical flow is smoothed by a Gaussian.')
    parser.add_argument('--zradius',type=int, default=4, help='The radius in Z of the window over wich the optical flow is smoothed by a Gaussian.')
    ...
    #Prepare optical flow objects
    spatial_size = 2*args.poly_radius
    ndim = 3 #dimensionality of the image
    qAbc = memory_efficient.QuadraticToAbc(ndim)
    basis = qAbc.basis
    applicability = memory_efficient.gaussian_applicability(spatial_size, ndim)
    cb = memory_efficient.CorrelationBand(tuple(ser.getFrameShape()), applicability, basis)
    #Normalized Separable correlation
    mSNC = memory_efficient.metrics_SNC(applicability, basis)
    #spatial averaging (by normalised separable correlation with a Gaussian)
    basis2 = np.zeros((ndim,1), np.int64)
    applicability2 = [np.ones(2*args.zradius+1, np.float32)] + memory_efficient.gaussian_applicability(2*args.radius+1, ndim-1)
    cb2 = memory_efficient.CorrelationBand(tuple(ser.getFrameShape()), applicability2, basis2, n_fields=ndim*(ndim+3)//2)
    mSNC2 = memory_efficient.metrics_SNC(cb2.applicability, cb2.basis)
    Ginv = mSNC2.central_Ginv()[0,0]

    #Create the objects that hold the correlation results and will perform optical flow
    results0 = memory_efficient.CorrelationResult(cb, mSNC)
    results1 = memory_efficient.CorrelationResult(cb, mSNC)
    ...
    with h5py.File(args.output+".h5", "a") as h5file:
        ...
        grp = h5file.create_group("texture_opticalflow")
        ...
        for ser in sers:
            ...
            for t, fr in ser.enumByFrame():
                #condition the present frame
                f1 = normalize_stack(fr, px, args.preblur).astype(np.float32)
                if t+ti==0:
                    results1.initialize(f1)
                else:
                    results0, results1 = results1, results0
                    displ = []
                    for z, gh in enumerate(cb2.generator(results1.displacement_matrices_generator(f1, results0, displYX[t+ti-1]))):
                        if z<args.minZ or z>=args.maxZ or interest[z-args.minZ].sum()==0:
                            continue
                        #restrict to the interest points
                        trimmed_gh = gh[args.radius:-args.radius, args.radius:-args.radius][interest[z-args.minZ]]
                        #normalize the results by inversed metrics
                        Gh = Ginv * trimmed_gh
                        #compute displacement only at interest points
                        displ.append(memory_efficient.Gh2displ(Gh[...,:-ndim,0], Gh[...,-ndim:,0]))
                ...
                interest = lattice3D & thresholded[args.minZ:args.maxZ, args.radius:-args.radius, args.radius:-args.radius]
                pts = np.column_stack(np.where(interest))
