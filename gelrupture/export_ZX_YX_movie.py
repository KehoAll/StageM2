...
def normalize_stack(stack, px, preblur=1., profile=None):
    ...
    f0 = ndi.gaussian_filter(stack.astype(float), preblur*px[-1]/px[:])
...
if __name__ == '__main__':
    ...
    if args.length is None:
        args.length = nbFrames - args.start_time
    ...
    to_update['ZXslice'] = axs[0,0].imshow(
        asnumpy(stack[:,args.Y-5:args.Y+6,:].mean(1)),
    ...
    to_update['ZYslice'] = axs[1,1].imshow(
        asnumpy(stack[:,:,args.X-5:args.X+6].mean(2).T),
        cmap = cmap, norm=color_norm,
        extent=(
            0, shape[0]*px[0],
            0, shape[1]*px[1],
    ...
    def update_graph(t):
        stack = normalize_stack(xp.array(ser.getFrame(t)), px, preblur=0, profile=profile)
        to_update['ZXslice'].set_data(asnumpy(stack[:,args.Y-5:args.Y+6,:].mean(1)))
        to_update['YXslice'].set_data(asnumpy(stack[args.Z]))
        to_update['ZYslice'].set_data(asnumpy(stack[:,:,args.X-5:args.X+6].mean(2).T))
